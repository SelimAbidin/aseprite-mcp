...
The implemented named methods are `get_status`, `get_document`, `create_sprite`, and `open_sprite`. Filesystem paths are validated and canonicalized by the Node process before a request reaches the extension.
